...
class GenerateQuote(object):
	...
	def printquote(my, charaName, updateFlag=False):
		...
		return my.translateLink(endText)
...
def test():
	...
	if wikiText.find('Quotes') == -1 or wikiText.find('{{Knightquote') == -1:
		...
		text = GenerateQuote.translateLink(wikiText)
		comment = 'Fixed external link in quotes'
	else:
...
def main():
	site = pywikibot.Site()
	flowerKnightBaseList = pywikibot.Category(site, 'Category:Is_base_form').articles()
	flowerKnightAltList  = pywikibot.Category(site, 'Category:Is_alternate_form').articles()
	flowerKnightCoList  = pywikibot.Category(site, 'Category:Collaboration').articles()
